<template>
  <div>
    <Card>
      ...
      <Card>
        <div slot="title"><p>维护笔记</p></div>
        <Form label-position="right" :label-width="185" label-colon>
          <Row>
            <Col :md="12">
              <FormItem label="维护客户">
                <div class="company-box" style="overflow: hidden">
          ...
          <Row v-for="(item, index) in detailData.surveyInfo" :key="index">
            <Col :md="12">
              <FormItem :label="item.title">
                <Input :type="item.question_id === '12' || item.question_id === '13' ? 'textarea' : 'text'" :value="item.value" readonly />
              </FormItem>
            </Col>
          </Row>
        </Form>
...
export default {
  ...
  methods: {
    ...
    getSurvey() {
      this.$api.attendanceSurveyList({checkWay: 'maintain'}).then((res) => {
        if(res.code === 0) {
