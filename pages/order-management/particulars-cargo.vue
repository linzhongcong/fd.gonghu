<template>
	<div class="content">
		...
		<div class="box">
			<!-- <Card> -->
			<div style="text-align:right"><Button @click="goBack">返回</Button></div>
			<!-- </Card> -->
		</div>
		<!-- </Affix> -->
		...
		<div class="box">
			<Card>
				...
				<div style="padding-top:1rem">
					<Card>
						<Row style="padding-bottom:1rem" v-for="(item, index) in data.auditLog" :key="index">
							<Col span="6"> 审核人：{{ item.created_by }} </Col>
							<Col span="6">
								审核结果：{{ item.audit_pass == '0' ? '未审核' : item.audit_pass == '1' ? '审核通过' : '审核失败' }}
							</Col>
							<Col span="6"> 审核时间：{{ timestampToTime(item.created_at) }} </Col>
							<Col span="7"
								><br />
								审核意见：{{ item.opinion }}
							</Col>
