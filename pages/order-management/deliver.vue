	...
	<div>
		...
		<Modal v-model="detailsList.modal" title="详情" width="1200">
			...
			<div class="rel">
										...
										<td>
											<div class="ivu-table-cell">
												<span
													v-text="
														detailsList.data.parentOrder
															? detailsList.data.parentOrder.partner_policy_id
															: detailsList.data.partner_policy_id
													"
												></span>
											</div>
										</td>
				...
				<div class="ivu-table-wrapper item" v-if="detailsList.data.parent_order_id === '0'">
					<!--未拆订单详情-->
					<Table border :columns="productCloumns" :data="detailsList.data.allProduct"></Table>
				</div>
				...
				<div class="item" v-if="detailsList.data.parent_order_id !== '0'">
						...
						<div class="ivu-table-body">
							<table cellspacing="0" cellpadding="0" border="0" width="100%">
								...
								<tr class="ivu-table-row">
									...
									<td colspan="3">
										<span
											v-if="detailsList.data.delivery"
											v-text="formatDeliveryMode(detailsList.data.delivery.delivery_mode)"
										></span>
									</td>
								</tr>
								<tr class="ivu-table-row">
									...
									<td colspan="3">
										<span
											v-if="detailsList.data.delivery"
											v-text="detailsList.data.delivery.delivery_status === 'delivery_finish' ? '是' : '否'"
										></span>
									</td>
									<td colspan="2" class="head-bg">
									...
									<td colspan="3">
										<span
											v-if="detailsList.data.delivery"
											v-text="formatDate(detailsList.data.delivery.updated_at)"
										></span>
									</td>
								</tr>
								<tr class="ivu-table-row">
									...
									<td colspan="8">
										<span v-if="detailsList.data.delivery" v-text="detailsList.data.delivery.shortage_info"></span>
									</td>
								</tr>
								<tr class="ivu-table-row">
									<td colspan="2" class="head-bg">
									...
									<td colspan="8">
										<span v-if="detailsList.data.delivery" v-text="detailsList.data.delivery.remark"></span>
									</td>
								</tr>
							</table>
						</div>
						...
						<Col :md="6">
							<FormItem label="发货状态">
								<template v-if="sendDetails.data.delivery.delivery_status">
									<Input v-model="sendDetails.data.delivery.delivery_status" disabled />
								</template>
						...
						<Col :md="6">
							<FormItem label="发货方式">
								<template v-if="sendDetails.data.delivery.delivery_mode">
									<Input v-model="sendDetails.data.delivery.delivery_mode" disabled />
								</template>
								<Input v-else value="暂无" disabled />
							</FormItem>
						</Col>
...
export default {
	...
	data() {
		return {
			...
			translateStatus: [
				{ value: 'auditPending', label: '待初审' },
				{ value: 'auditFailure', label: '初审未通过' },
				{ value: 'auditRecheckPending', label: '待复审' },
				{ value: 'auditRecheckPass', label: '复审通过' },
				{ value: 'auditRecheckFailure', label: '复审未通过' },
			],
			originList: [
								...
								'a',
								{
									on: {
										click: () => {
											...
											this.logistics = []
											this.$api
												.orderDeliveryDetails(params.row.id)
												.then(res => {
	...
	methods: {
		...
		// 筛选订单状态
		filterStatus(status) {
			let data = this.translateStatus
			let str = data.filter(item => {
				if (status.indexOf(item.value) > -1) {
					return item.label
				}
			})
			return str.length == 0 ? '' : str[0].label
		},
		// 获取列表
		getList(type) {
			if (type == 'search') {
		...
		viewSet() {
			...
			let status = this.checkList[0].delivery_status
			if (status === '已发货') {
		...
		submitSet() {
			...
			this.setList.data.orderLogistics.courierCompany = this.setList.data.courier_company
		...
		orderSplit() {
			...
			this.$router.push({
				path: './order-split',
				query: { id: this.checkList[0].id, type: 'add' },
			})
		},
		// 审核不通过修改子订单
		editOrderSplit(fatherOrderId) {
			...
			this.$router.push({
				path: './order-split',
				query: { id: fatherOrderId, type: 'edit' },
			})
		},
		//打开拆单审核模态框
		openOrderSplitModal() {
			...
			this.$api
				.orderDeliverDetail({ id: this.checkList[0].id })
				.then(res => {
					if (res.code === 0) {
		...
		submitHandle() {
			...
			if (this.handleList.params.dispose_status !== '') {
		...
		changeCourierCompany(arr, index) {
			for (let i in this.fmsService) {
				if (arr.value == this.fmsService[i].courier_company_id) {
					...
					this.setList.orderLogistics[index].courierCompanyId = arr.value
				}
			}
		},
...
.delivery-timeline /deep/ .ivu-timeline-item-content {
	position: relative;
}
...
.modal-height /deep/ .ivu-modal-body {
	max-height: 600px;
	overflow: hidden;
	overflow-y: auto;
}
