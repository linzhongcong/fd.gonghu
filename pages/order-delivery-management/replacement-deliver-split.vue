  ...
  <Card style="padding: 0 30px 30px">
    ...
    <Modal v-model="submitConfirmModal.modal" title="提示" width="600">
      <div style="padding: 20px 10px">
        <Icon type="md-alert" color="#faad14" size="30"/>
        请确认拆单结果再提交，尽量避免多次修改操作！
      </div>
      <div slot="footer">
        <Button type="default" size="large" @click="submitConfirmModal.modal = false">检查一下</Button>
        <Button type="primary" size="large" :loading="submitConfirmModal.btn_loading"
                @click="orderSplitSubmitConfirm()">确认提交
        </Button>
      </div>
    </Modal>
...
<script>
  export default {
    name: "replacement-deliver-split",
    data() {
      return {
    ...
    methods: {
      ...
      getOrderDetail() {
        ...
        this.$api.reissueDeliveryView({id: this.$route.query.id})
          .then((res) => {
            // 订单头部信息
      ...
      orderSplitSubmitConfirm() {
        ...
        this.$api.reissueSeparateOrder(postData)
          .then(res => {
            if (res.code === 0) {
