<template>
	<div>
		...
		<Modal v-model="detailsList.modal" :loading="detailsList.loading" title="详情" width="1200">
			<div class="rel">
				<!-- 订单详情 start -->
										...
										<td>
											<div class="ivu-table-cell">
												<span
													v-text="
														detailsList.data.parentOrder
															? detailsList.data.parentOrder.merchant_name
															: detailsList.data.merchant_name
													"
												></span>
											</div>
										</td>
				...
				<template v-if="unissuedData.unissuedProducts.data.length">
					<p class="text-align-center-blod">原订单未发清单</p>
					<Table border :columns="productCloumns" :data="unissuedData.unissuedProducts.data"></Table>
				</template>

				<template v-if="unissuedData.repurchaseProduct.data.length">
					<p class="text-align-left">换购清单</p>
					<Table border :columns="productCloumns" :data="unissuedData.repurchaseProduct.data"></Table>
				</template>

				<template v-if="unissuedData.backPoint.data.length">
					<p class="text-align-left">返点清单</p>
					<Table border :columns="productCloumns" :data="unissuedData.backPoint.data"></Table>
				</template>

				<template v-if="unissuedData.matchingProduct.data.length">
					<p class="text-align-left">配赠清单</p>
					<Table border :columns="productCloumns" :data="unissuedData.matchingProduct.data"></Table>
				</template>

				<template v-if="unissuedData.giftProduct.data.length">
					<p class="text-align-left">赠品清单</p>
					<Table border :columns="productCloumns" :data="unissuedData.giftProduct.data"></Table>
				</template>
				...
				<template v-if="reissueData.order.data.length">
					<p class="text-align-center-blod">补发产品清单</p>
					<Table
						border
						:columns="productCloumns"
						:data="reissueData.order.data"
					...
					<div v-else style="position: relative">
						...
						<a
							...
							>查看其他子订单</a
						>
						<Table border :columns="productCloumns" :data="reissueData.order.data"></Table>
					</div>
				</template>

				<template>
					<p class="text-align-center-blod">发货信息</p>
					<div class="ivu-table-wrapper">
						<div class="ivu-table ivu-table-small ivu-table-border">
							<div class="ivu-table-body">
								<table cellspacing="0" cellpadding="0" border="0" width="100%">
									...
									<tr class="ivu-table-row">
										...
										<td colspan="3">
											<span v-text="formatDate(detailsList.data.delivery.confirm_delivery_time)"></span>
										</td>
										<td colspan="2" class="head-bg">
										...
										<td colspan="3">
											<span
												v-text="!detailsList.data.delivery ? '暂无' : detailsList.data.delivery.shortage_info"
											></span>
										</td>
									</tr>
									<tr class="ivu-table-row">
										...
										<td colspan="3">
											<span
												v-text="!detailsList.data.delivery ? '暂无' : detailsList.data.delivery.logistics_id"
											></span>
										</td>
									</tr>
									<tr class="ivu-table-row">
				...
				<template v-if="detailsList.data.separateAuditLogItems">
					<p class="text-align-center-blod">审核信息</p>
					<div class="ivu-table-wrapper">
			...
			<div class="rel">
				...
				<Table ref="myTable" border :columns="deliveryColumns" :data="setList.orderLogistics" class="delivery-wrapper">
					<template slot="orderLogisticsId" slot-scope="props">
						<Input v-model="setList.orderLogistics[props.row._index].orderLogisticsId" :maxlength="50"></Input>
						<!-- <Input v-model="props.row.orderLogisticsId" :maxlength="50"></Input> -->
					</template>
					<template slot="company" slot-scope="props">
						<Select
							v-model="setList.orderLogistics[props.row._index].courierCompanyId"
							label-in-value
						...
						<!-- <Select v-model="props.row.courierCompanyId" label-in-value @on-change="(arr) => changeCourierCompany(arr,props.index)">
              <Option v-for="item in fmsService" :value="item.courier_company_id" :key="item.courier_company_id"
                      :label="item.courier_company" >{{ item.courier_company }}
              </Option>
            </Select> -->
					</template>
					<template slot="content" slot-scope="props">
						<Input v-model="setList.orderLogistics[props.row._index].content" :maxlength="100"></Input>
		...
		<Modal v-model="orderSplitAudit.modal" title="拆单审核" width="1200">
			...
			<div class="rel">
				...
				<template v-if="orderSplitAudit.data.unissuedProducts.length">
					<p class="text-align-center-blod">原订单未发清单</p>
					<Table border :columns="productCloumns" :data="orderSplitAudit.data.unissuedProducts"></Table>
				</template>

				<template v-if="orderSplitAudit.data.repurchaseProduct">
					<p class="text-align-left">换购清单</p>
					<Table border :columns="productCloumns" :data="orderSplitAudit.data.repurchaseProduct"></Table>
				</template>

				<template v-if="orderSplitAudit.data.backPoint">
					<p class="text-align-left">返点清单</p>
					<Table border :columns="productCloumns" :data="orderSplitAudit.data.backPoint"></Table>
				</template>

				<template v-if="orderSplitAudit.data.matchingProduct">
					<p class="text-align-left">配赠清单</p>
					<Table border :columns="productCloumns" :data="orderSplitAudit.data.matchingProduct"></Table>
				</template>

				<template v-if="orderSplitAudit.data.giftProduct">
					<p class="text-align-left">赠品清单</p>
					<Table border :columns="productCloumns" :data="orderSplitAudit.data.giftProduct"></Table>
				</template>
						...
						<Col :md="6">
							<FormItem label="发货方式">
								<template v-if="sendDetails.data.delivery.delivery_mode">
									<Input v-model="deliveryNmode" disabled />
								</template>
								<Input v-else value="暂无" disabled />
							</FormItem>
						</Col>
...
export default {
		...
		return {
			...
			listData: {
				columns: [
					...
					{ key: 'merchant_name', title: '商家名称', minWidth: 180, align: 'center' },
					{
						key: 'order_sn',
						title: '补发订单编号',
						minWidth: 180,
	...
	methods: {
		...
		viewDetails(id) {
			...
			this.$api.reissueDeliveryView({ id: id }).then(res => {
				if (res.code === 0) {
					...
					for (let key in this.unissuedData) {
						this.unissuedData[key].data = res.data[key] || []
					}
					this.reissueData.order.data = allProduct || []
				}
			})
		},
		// 清空详情未发补发订单数据
		...
		submitSet() {
			for (let i in this.setList.orderLogistics) {
				...
				// this.setList.data.orderLogistics[i].orderLogisticsId = this.setList.orderLogistics[i].order_logistics_id;
				// this.setList.data.orderLogistics[i].courierCompanyId = this.setList.orderLogistics[i].courier_company_id;
			}
			...
			// this.setList.data.deliveryMode = this.setList.data.delivery_mode;
		...
		// 下载出货单
		downlodaSales(id) {
			this.$api.reissueDeliveryExport({ id: id }).then(res => {
				if (res.code === 0) {
		...
		orderSplit() {
			...
			this.$router.push({ path: './replacement-deliver-split', query: { id: this.checkList[0].id, type: 'add' } })
		},
		// 审核不通过修改子订单
		editOrderSplit(fatherOrderId) {
			this.detailsList.modal = false
			this.$router.push({ path: './replacement-deliver-split', query: { id: fatherOrderId, type: 'edit' } })
		},
		//打开拆单审核模态框
		openOrderSplitModal() {
			if (this.checkList.length !== 1) {
			...
			this.$api
				.reissueDeliveryView({ id: this.checkList[0].id })
				.then(res => {
					if (res.code === 0) {
		...
		orderSplitAuditSubmit() {
			...
			this.$api
				.reissueSeparateOrderAudit(postData)
				.then(res => {
					if (res.code === 0) {
	...
	computed: {
		deliveryNmode: {
			...
			set	: function (value) {
				this.sendDetails.data.delivery.delivery_mode == value
			}
		}
	},
	activated() {
