        ...
        <Col :sm="24">
          <Form ref="search" :model="search" :rules="formRules" class="search">
            <Col>
              ...
              <!--                            </FormItem>-->
              <!--                            <FormItem prop="date">-->
              <!--                                <DatePicker v-model="search.order_time" @on-change="estimateTime" type="date" clearable placeholder="请选择下单时间"></DatePicker>-->
              <!--                            </FormItem>-->
              <FormItem prop="date">
              ...
              <FormItem prop="merchant_name">
                <Input v-model="search.merchant_name" @keydown.native.enter.prevent="getList('search')" placeholder="商家名称" clearable/>
              </FormItem>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {key: 'merchant_name', title: '商家名称', minWidth: 180, sortable: true, align: 'center'},
          {
            key: 'order_time',
            title: '结款方式',
            minWidth: 110,
          ...
          },
          {
            ...
            align: 'center',
            render: (h, params) => {
              return h('div', {}, this.orderType[params.row.order_form]);
            }
          },
          {
            key: 'order_time',
          ...
          {key: 'miniprogram_status_desc', title: '小程序订单状态', width: 100, align: 'center'},
          {key: 'import_mode_desc', title: '付款方式', width: 100, align: 'center'},
          {key: 'import_at_desc', title: '付款时间', width: 100, align: 'center'},
        ],
        // table数据
        formData: [],
        // 搜索表单验证规则 清空按钮作用
    ...
    methods: {
      ...
      getList() {
        ...
        this.$api.receiptCashList(params)
          .then(res => {
            this.loading = false;
      ...
      viewDetails(id, order_sn) {
        ...
        this.$api.receiptCashReceiptView({id: id})
          .then(res => {
            if (res.code === 0) {
      ...
      applyFor() {
        ...
        this.$router.push({
          ...
          query: {id: checkList[0].id, type: 'cash'}
        });
      },
      /***
       * 重置搜索按钮
      ...
      getherMoney() {
        ...
        if (checkList[0].receivables_status !== 'wait_receipt') {
          this.$Message.warning('待付款状态下才可以进行收款!');
          return;
        }
        let query = {id: checkList[0].id};
        this.$router.push({
          path: '/receipt/cash-edit',
          query: query
        });
      },
      ...
      estimateTime(estimateTimeArr) {
        this.search.order_time = estimateTimeArr;
        this.getList();
      },
