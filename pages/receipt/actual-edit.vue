                ...
                <Col span="23" class="border-row">
                    ...
                    <div class="basic">
                        <Row>
                            ...
                            <Col :sm="8" :xs="24">
                                <FormItem label="项目：" :prop="'receipt.' + index + '.product'" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                        ...
                        <Row>
                            <Col :sm="8" :xs="24">
                                <FormItem label="汇款户名：" :prop="'receipt.' + index + '.remittance_name'" :rules="{required: true, message: '该选项不能为空！', trigger: 'blue', type:'string'}" :label-width="90">
                                    <AutoComplete
                            ...
                            <Col :sm="8" :xs="24">
                                <FormItem label="汇款账号：" :prop="'receipt.' + index + '.remittance_account'" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                                    <AutoComplete
                            ...
                            <Col :sm="8" :xs="24">
                                <FormItem label="汇入方式：" :prop="'receipt.' + index + '.import_mode'" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                        ...
                        <Row>
                            <Col :sm="8" :xs="24">
                                <FormItem label="汇入户名：" :prop="'receipt.' + index + '.import_name'" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                                    <Input v-model="item.import_name" disabled placeholder="请输入汇入户名"></Input>
                                </FormItem>
                            </Col>
                            <Col :sm="8" :xs="24">
                                <FormItem label="汇入账号：" :prop="'receipt.' + index + '.import_account'" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                        ...
                        <Row>
                            <Col :sm="8" :xs="24">
                                <FormItem label="汇入时间：" :prop="'receipt.' + index + '.import_at'" :rules="{required: true, message: '该选项不能为空！', trigger: 'change', type: 'date'}" :label-width="90">
                        ...
                        <Row>
                            <Col :sm="8" :xs="24">
                                <FormItem label="上传凭证" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                                    <Upload @click.native="limtUpload(index)" name="UploadForm[file]" :action="fileUploadMaterial" :headers="fileUploadHeaders"
...
export default {
            ...
            this.$refs[name].validate((valid) => {
                if (!valid) {
                    ...
                    if (status) {
                        ...
                        this.$api.realSalesDouble(params)
                            .then(res => {
    ...
    mounted () {
        let checkInfo = localStorage.getItem('actually-order');
     ...
     */
    destroyed () {
        localStorage.removeItem('actually-order');
    }
};
</script>
