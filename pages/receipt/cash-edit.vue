<template>
  <Form ref="formData" :model="formData" :rules="formRules" label-position="left">
    <Card>
            ...
            <Row>
              <Col :sm="8" :xs="24">
                <FormItem label="订单号" :label-width="90">
                  <Input v-model="formData.order_sn" disabled></Input>
                </FormItem>
              </Col>
              <Col :sm="8" :xs="24">
                <FormItem label="合作商" :label-width="90">
                  <Input v-model="formData.merchant_name" disabled></Input>
                </FormItem>
              </Col>
              <Col :sm="8" :xs="24">
                <FormItem label="订单金额" :label-width="90">
                  <Input v-model="formData.total_order_amount" disabled></Input>
                </FormItem>
              </Col>
            </Row>
          </div>
      ...
      <Row type="flex" v-for="(item, index) in formData.receipt" :key="index">
        <Col span="23" class="border-row">
          ...
          <div class="basic">
            <Row>
              ...
              <Col :sm="8" :xs="24">
                <FormItem label="项目：" :prop="'receipt.' + index + '.product'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
            ...
            <Row>
              <Col :sm="8" :xs="24">
                <FormItem label="汇款户名：" :prop="'receipt.' + index + '.remittance_name'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'blue', type:'string'}"
                          :label-width="90">
                  <AutoComplete
                  :value="item.remittance_name"
                  @on-search="v=> {queryRemittanceName(v, index, 'name')}"
                  @on-change="v => {getRemittanceName(v, index, 'name')}"
                  @on-select="v => {getRemittanceName(v, index, 'name')}"
                  placeholder="请输入汇款户名"
                  style="">
                    <Option v-for="(item,index) in queryremittanceNameList" :value="item.name" :key="'rename' + index">
                      {{ item.name }}
                    </Option>
                  </AutoComplete>
                </FormItem>
              ...
              <Col :sm="8" :xs="24">
                <FormItem label="汇款账号：" :prop="'receipt.' + index + '.remittance_account'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                  <AutoComplete
                  :value="item.remittance_account"
                  @on-search="v=> {queryRemittanceName(v, index, 'account')}"
                  @on-change="v => {getRemittanceName(v, index, 'account')}"
                  @on-select="v => {getRemittanceName(v, index, 'account')}"
                  placeholder="请输入汇款账号">
                    <Option v-for="(item,index) in queryremittanceNameList" :value="item.name" :key="'rename' + index">
                      {{ item.account }}
                    </Option>
              ...
              <Col :sm="8" :xs="24">
                <FormItem label="汇入方式：" :prop="'receipt.' + index + '.import_mode'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                  <Select v-model="item.import_mode" style="width:200px"
                          @on-change="(val) => { changePayType(val, item) }">
            ...
            <Row>
              <Col :sm="8" :xs="24">
                <FormItem label="汇入户名：" :prop="'receipt.' + index + '.import_name'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                  <Input v-model="item.import_name" placeholder="请输入汇入户名" disabled></Input>
                </FormItem>
              </Col>
              <Col :sm="8" :xs="24">
                <FormItem label="汇入账号：" :prop="'receipt.' + index + '.import_account'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}" :label-width="90">
                  <!-- <Input v-model="item.import_account" placeholder="请输入汇入账号："></Input> -->
              ...
              <Col :sm="8" :xs="24">
                <FormItem label="汇入金额："
                          :prop="'receipt.' + index + '.import_amount'"
                          :rules="[
            ...
            <Row>
              <Col :sm="8" :xs="24">
                <FormItem label="汇入时间：" :prop="'receipt.' + index + '.import_at'"
                          :rules="{required: true, message: '该选项不能为空！', trigger: 'change', type: 'date'}"
                          :label-width="90">
                  <DatePicker v-model="item.import_at" type="date" style="width: 200px"
                              placeholder="请选择汇入时间"></DatePicker>
                </FormItem>
              </Col>
            </Row>
            <Row>
              <Col :sm="8" :xs="24">
                <FormItem label="上传凭证" :rules="{required: true, message: '该选项不能为空！', trigger: 'blur'}"
                          :label-width="90">
                  <Upload @click.native="limtUpload(index)" name="UploadForm[file]" :action="fileUploadMaterial"
                          :headers="fileUploadHeaders"
                          :default-file-list="formData.receipt[index].fileItems"
                          :max-size="51200"
        ...
        <Col span="1">
          <Button v-if="index > 0" icon="md-remove" @click="removeItem(index)"></Button>
          ...
          .then(res => {
            if (res.code === 0) {
              ...
              data = {
                ...
                receipt: [
                  {created_by: res.data.created_by, fileItems: [], import_name: '广州慕可生物科技有限公司'}
                ]
              };
              this.formData = data;
        ...
        this.$refs[name].validate((valid) => {
          if (!valid) {
            ...
            if (status) {
              ...
              this.$api.receiptCashReceipt(params)
                .then(res => {
