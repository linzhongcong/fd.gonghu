          ...
          <Form ref="search" :model="search" :rules="formRules" class="search">
            <Col>
              ...
              <FormItem style="float:right">
                ...
                <Col :sm="12">
                  <Button icon="md-repeat" @click="handReset('search')" size="large">重置</Button>
                </Col>
              </FormItem>
            </Col>
          </Form>
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          {key: 'merchant_name', title: '商家名称', minWidth: 200, sortable: true, align: 'center'},
          {
            ...
            align: 'center',
            render: (h, params) => {
              return h('div', {}, this.translatePayment[params.row.payment_method]);
            }
          },
          {key: 'payment_days', title: '账期', minWidth: 80, sortable: true, align: 'center'},
          {key: 'payment_at', title: '结款日', minWidth: 90, sortable: true, align: 'center'},
          {
            key: 'order_sn',
            title: '订单号',
            minWidth: 180,
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('a', {
                ...
                style: {
                  color: this.paymentWarning(params.row.receivables_status, params.row.wait_receipt_hint_at)
                }
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('div', {
                style: {
                  color: params.row.overdue_desc == '是' ? 'red' : ''
                }
              }, params.row.overdue_desc);
            }
          },
          {
            key: 'status',
          ...
          {key: 'miniprogram_status_desc', title: '小程序订单状态', minWidth: 100, sortable: true, align: 'center'},
          {key: 'import_mode_desc', title: '付款方式', minWidth: 100, sortable: true, align: 'center'},
          {key: 'import_at_desc', title: '付款时间', minWidth: 100, sortable: true, align: 'center'},
    ...
    methods: {
      ...
      getList() {
        ...
        this.$api.receiptMonthly(params)
          .then(res => {
            this.loading = false;
      ...
      viewDetails(id, order_sn) {
        ...
        this.$api.receiptMonthlyReceipt({id: id})
          .then(res => {
            if (res.code === 0) {
      ...
      getherMoney() {
        ...
        localStorage.setItem('monthly-order', JSON.stringify(order));
        this.$router.push({
          path: '/receipt/month-edit'
        });
      },
      // 退款申请
      applyFor() {
        ...
        this.$router.push({
          ...
          query: {id: checkList[0].id, type: 'month'}
        });
      },
      /***
       * 收款状态转化中文显示
      ...
      paymentWarning(status, time) {
        // 判断是否为待付款
        if (status === 'wait_receipt') {
          let paymentTime = Number(time);
          let nowTime = new Date().getTime(); // 获取当前时间
          if (paymentTime === null || paymentTime === '' || paymentTime === '0') {
            return false;
          } else {
            if (parseInt(nowTime / 1000) > paymentTime && paymentTime !== 0) {
              return '#d93026';
            }
            ;
          }
    ...
    activated() {
       ...
       * 初始化列表数据、所有权数据
       */
      let receivables_status = this.$route.query.receivables_status
      receivables_status && (this.search.receivables_status = receivables_status)
      !receivables_status && (this.search.receivables_status = '')
