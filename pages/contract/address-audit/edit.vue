<template>
  <Card>
    <Row slot="title" type="flex" justify="end">
      <Button class="title-btn" @click="goBack">滚回</Button>
      <Button
        type="primary"
        class="title-btn"
        ...
        >
          ...
          <Row>
            <Col :md="8">
              <FormItem label="系统/门店名称：" prop="systemInfoId">
                <Select
                  ...
                  :loading="remoteLoading"
                  :placeholder="
                    formData.systemInfo.name || '请选择系统/门店名称'
                  "
                >
                  <Option
                    v-for="(item, index) in systemInfoList"
                    :value="item.id"
                    :label="item.name"
                    :key="index"
                  ></Option>
            ...
            <Col :md="8">
              <FormItem label="客户类型：">
                <Input
                  :value="formatMerchantType(formData.merchantType)"
                  disabled
                />
              </FormItem>
            </Col>
          ...
          <Row>
            <Col :md="8">
              <FormItem label="门店名称：" prop="name">
                <Input
                  ...
                  @on-change="() => (facadeNameFlag = false)"
                  placeholder="请输入门店名称"
                />
                <div v-show="facadeNameFlag" class="ivu-form-item-error-tip">
                  门店名称已存在
                </div>
              </FormItem>
            </Col>
            <Col :md="8">
              <FormItem label="进驻品牌：" prop="inBrands">
                <Select
                  v-model="formData.inBrands"
                  placeholder="请选择进驻品牌"
                  multiple
                >
                  <Option
                    v-for="item in inBrandsList"
                    :value="item.value"
                    :key="item.value"
                    :label="item.label"
                  ></Option>
            ...
            <Col :md="8">
              <FormItem label="经营状态" prop="manageStatus">
                <Select v-model="formData.manageStatus" placeholder="请选择">
                  <Option
                    v-for="item in manageStatusOption"
                    :value="item.value"
                    :key="item.value"
                    :label="item.label"
                  ></Option>
            ...
            <Col :md="8">
              <FormItem label="门店商品类型：" prop="goodsType">
                <Select v-model="formData.goodsType">
                  <Option
                    v-for="(item, index) in goodsTypeOption"
                    :value="item.value"
                    :label="item.label"
                    :key="index"
                  ></Option>
            ...
            <Col :md="8">
              <FormItem label="BA情况：" prop="hasBa">
                <Select v-model="formData.hasBa">
                  <Option
                    v-for="(item, index) in hasBaOption"
                    :value="item.value"
                    :label="item.label"
                    :key="index"
                  ></Option>
            ...
            <Col :md="8">
              <FormItem label="门店商圈：" prop="tradeArea">
                <Select v-model="formData.tradeArea">
                  <Option
                    v-for="(item, index) in tradeAreaOption"
                    :value="item.value"
                    :label="item.label"
                    :key="index"
                  ></Option>
            ...
            <Col :md="8">
              <FormItem label="商圈级别：" prop="tradeAreaLevel">
                <Select v-model="formData.tradeAreaLevel">
                  <Option
                    v-for="(item, index) in tradeAreaLevelOption"
                    :value="item.value"
                    :label="item.label"
                    :key="index"
                  ></Option>
  ...
  methods: {
        ...
        state: {
          add: async (params) => {
            ...
            try {
              ...
              error.message === "门店名称已存在" &&
                (this.facadeNameFlag = true); // 次判断内容需要与后端保持同步，一端修改及时通知另一端
              this.$Notice.error({
                title: error.code,
                desc: error.message,
              });
    ...
    async queryWorker(name) {
      ...
      try {
        ...
        const workerList = this.workerList,
          selectedIds = [
            ...new Set(
              workerList.map((item) => (item.selected ? item.id : null))
            ),
          ]; // 已选中的选项的id
        this.workerList = [
          ...this.workerList.filter((item) => selectedIds.includes(item.id)),
          ...res.items.filter((item) => !selectedIds.includes(item.id)),
        ];
        this.remoteLoading = false;
      } catch (error) {
        this.remoteLoading = false;
    ...
    selectOption(value, type) {
      switch (type) {
        case "company":
          ...
          break;
        case "worker":
          this.workerList.forEach(
            (item) => (item.selected = value.includes(item.id))
          );
          break;
        default:
          break;
      ...
      try {
        ...
        if (res.code === 0) {
          ...
          if (+data.provinceId && +data.cityId && +data.countyId)
            data.area = [data.provinceId, data.cityId, data.countyId];
    ...
    selectArea(value, selectedData) {
      this.formData.provinceId =
        (selectedData[0] && selectedData[0].value) || undefined;
      this.formData.province =
        (selectedData[0] && selectedData[0].label) || undefined;
      this.formData.cityId =
        (selectedData[1] && selectedData[1].value) || undefined;
      this.formData.city =
        (selectedData[1] && selectedData[1].label) || undefined;
      this.formData.countyId =
        (selectedData[2] && selectedData[2].value) || undefined;
      this.formData.county =
        (selectedData[2] && selectedData[2].label) || undefined;
    },
