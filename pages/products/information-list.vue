      ...
      <Row slot="title">
        <Col :sm="24" @keydown.native.enter.prevent="getList('search')">
          <Form ref="search" :model="search" class="search">
            <FormItem>
              <Input v-model="search.name" placeholder="资讯名称" clearable/>
            </FormItem>
            <FormItem><Input v-model="search.created_by" placeholder="创建人" clearable/></FormItem>
        ...
        <Col :sm="24">
          <router-link to="/products/information-list-add">
            <Button type="primary">新增资讯</Button>
          </router-link>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {key: 'name', title: '资讯名称', minWidth: 100, sortable: true, align: 'center'},
          {
            key: 'name', title: '资讯封面', minWidth: 110, sortable: true, align: 'center',
            render: (h, params) => {
              return h('a', {
                on: {
                  click: () => {
                    let id = params.row.id
                    this.$api.informationListPreviewS({id: id})
                      .then(res => {
          ...
          {key: 'describe', title: '资讯描述', width: 150, align: 'center', align: 'center'},
    ...
    methods: {
      ...
      getList(data) {
        ...
        this.$api.informationListLndex(params)
          .then(res => {
            this.loading = false;
            if (res.items) {
              this.data = res.items;
      ...
      handleEdit(id) {
        this.$router.push({
          path: '/products/information-list-add',
          query: {id: id}
        });
      },
      ...
      // 删除
      handleDelete(id) {
        this.$api.informationListDele(id)
          .then(res => {
            this.getList()
          })
      },
        ...
        if (this.checkList.length == 0) {
          ...
          this.checkList.forEach(element => {
            ...
            if (element.status == '隐藏') {
              ...
              this.$api.informationListShow({idArr: params})
                .then(res => {
              ...
              this.$api.informationListNotShow({idArr: params})
                .then(res => {
                  this.$Message.success(res.message)
                  this.getList();
                })
