    ...
    <Modal v-model="modalAudit" width="970px" @on-cancel="colseModal()">
      ...
      <div style="height:350px;overflow-y: scroll">
        <table class="review-table">
          ...
          <tr>
            <td colspan="9">库存总值：{{ detailData.total_val }}</td>
          </tr>
          <tr>
...
<script>
  export default {
      ...
      return {
        ...
        listData: {
          columns: [
            ...
            {title: '库存单号', key: 'version', width: 180, align: 'center'},
    ...
    methods: {
      ...
      goEdit() {
        this.$router.push({path: '/store-order-management/store-order-review/order-edit'})
      },
      select(selection) {
      ...
      getList(type) {
        ...
        this.$api.stockReviewList(params).then((res) => {
      ...
      getDetail() {
        this.$api.stockReviewDetail({id: this.checkList[0].id}).then((res) => {
          if (res.code === 0) {
      ...
      submitAudit() {
        if (this.auditData.is_pass == '') {
          ...
          this.$api.stockReviewAudit(params).then((res) => {
            if (res.code === 0) {
      ...
      handleDelete(row) {
        if (row.audit_status == '审核通过') {
          ...
          this.$api.stockReviewDelete({id: row.id}).then((res) => {
            if (res.code === 0) {
      ...
      handleEdit(row) {
        if (row.audit_status === '审核通过') {
          ...
          this.$router.push({
            path: '/store-order-management/store-order-review/order-edit',
            query: {
              id: row.id
            }
          })
