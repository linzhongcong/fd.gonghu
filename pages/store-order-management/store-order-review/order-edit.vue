  ...
  <div>
    <Card>
      <Row slot="title" type="flex" justify="space-between">
        <Col><p style="line-height: 30px;height: 30px;">填写库存单</p></Col>
        <Col>
          <Button type="success" :loading="loadingSubmit" @click="submit()">提交</Button>
        </Col>
      </Row>
      <Row v-for="(data,index) in storeLists" :key="'store' + index">
        <Col :md="22" class="order">
          <Form label-position="right" :label-width="78">
              ...
              <Col :md="6">
                <FormItem label="商家名称" required>
                  <Select v-model="data.contractor_id" @on-query-change="queryContractorDebonce(...arguments, index)"
                          ...
                          clearable>
                    <Option v-for="(item, key) in storeListSelect[index].contractorList" :key="'contractor'+key"
              ...
              <Col :md="4">
                <FormItem label="门店名称" required>
                  <Select v-model="data.facade_id" @on-change="selectFacade(...arguments, index)"
                    ...
                    <Option v-for="(item, key) in storeListSelect[index].facadeList" :key="'facade'+key"
                            :value="item.id" :label="item.name"></Option>
                  </Select>
                </FormItem>
              </Col>
              <Col :md="14">
              ...
              <Col :md="6">
                <FormItem label="产品名称" required>
                  <Select v-model="item.product_id" @on-query-change="queryProductDebonce(...arguments, index, i)"
                          ...
                          clearable>
                    <Option v-for="(value, key) in storeListSelect[index].items[i].productList" :key="'product'+ key"
            ...
            <Row>
              <Col :md="12">
                <FormItem label="库存凭证" required>
                  <Upload name="file"
                          :action="fileUploadURL"
                          :headers="fileUploadHeaders"
                          :default-file-list="storeListSelect[0].defaultList"
        ...
        <Col :md="2" v-if="!orderId">
          <Button icon="md-add" v-if="index === 0" @click="addStore()"></Button>
          <Button icon="md-remove" v-if="storeLists.length>1" @click="removeStore(index)"></Button>
        </Col>
      </Row>
    </Card>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      addProduct(index) {
        ...
        let arr = {productList: []}
        this.storeLists[index].items.push(obj)
        this.storeListSelect[index].items.push(arr)
      },
      // 删除产品
      removeProduct(index, i) {
        this.storeLists[index].items.splice(i, 1)
        this.storeListSelect[index].items.splice(i, 1)
      ...
      getCount(num, price, index, i) {
        ...
        if (!isNaN(retailPrice)) {
          this.storeLists[index].items[i].value = Math.round(num * retailPrice * 10000) / 10000
          this.storeLists[index].items[i].number = num
          // 计算总货值
          this.calculateTotal(index)
        } else {
          this.storeLists[index].items[i].value = 0
        }
      },
      // 计算总货值
      calculateTotal(index) {
        this.storeLists[index].total_val = 0
        this.storeLists[index].items.map((item) => {
          this.storeLists[index].total_val += item.value
        })
      },
      addStore() {
        let obj = {
        ...
        this.storeLists.push(obj)
        this.storeListSelect.push(arr)
      },
      removeStore(index) {
        this.storeLists.splice(index, 1)
        this.storeListSelect.splice(index, 1)
      },
      submit() {
        let param = JSON.parse(JSON.stringify(this.storeLists))
        let valid = false
        param.map((item) => {
        ...
        if (valid) {
          ...
          if (!!this.orderId) {
            ...
            this.$api.stockReviewEdit(obj).then((res) => {
              if (res.code === 0) {
                this.goBack('编辑成功')
              }
            }).finally(() => {
            ...
            this.$api.stockReviewCreate({params: param}).then((res) => {
              if (res.code === 0) {
                this.goBack('添加成功')
              }
            }).finally(() => {
      ...
      fileSuccess(response, file, fileList, index) {
        if (response.code === 0) {
          this.storeLists[index].fileItems.push(response.data.fileUploadVo)
        }
      },
      fileRemove(file, fileList, index) {
        let fileIndex = this.storeLists[index].fileItems.findIndex((item) => {
          return file.name == item.name
        })
        this.storeLists[index].fileItems.splice(fileIndex, 1)
      },
      goBack(tip) { //返回列表页
        this.$Message.success(tip)
        this.$store.commit('removeTag', this.$route.name)
        this.$router.push({path: '/store-order-management/store-order-review/review'})
      },
      queryProduct(query, index, i) { // 产品查询
        this.$api.orderReviewQueryProduct({name: query}).then((res) => {
          if (res.code === 0) {
            this.storeListSelect[index].items[i].productList = res.data
          }
        })
      },
      queryContractor(query, index) {
        ...
        this.$api.orderReviewQueryContractor({name: query}).then((res) => {
          if (res.code === 0) {
            this.storeListSelect[index].contractorList = res.data
          }
        }).finally(() => {
      ...
      selectContractor(val, index) { // 选择商家搜索出当前商家的所用门店
        if (!!val) {
          ...
          this.$api.facadeQuery({id: val}).then((res) => {
            if (res.code === 0) {
              this.storeListSelect[index].facadeList = res.data
            }
          }).finally(() => {
            this.loadingSelect = false
          })
        } else {
          this.storeLists[index].facadeAddress = ''
          this.storeLists[index].facade_id = ''
          this.storeListSelect[index].facadeList.length = 0
        }
      },
      selectFacade(val, index) { // 选择门店获取地址
        if (this.storeListSelect[index].facadeList.length > 0) {
          this.storeListSelect[index].facadeList.map((item) => {
            if (item.id === val) {
              this.storeLists[index].facadeAddress = item.address
            }
          })
        }
      },
      selectProduct(val, index, i) {
        if (val) {
          if (this.storeListSelect[index].items[i].productList.length > 0) {
            this.storeListSelect[index].items[i].productList.map((item) => {
              if (item.pro_id === val) {
                this.storeLists[index].items[i].retail_price = item.price
                this.storeLists[index].items[i].number = 0
                this.storeLists[index].items[i].value = 0
                this.calculateTotal(index)
              }
            })
    ...
    mounted() {
      ...
      if (!!this.orderId) {
        this.$api.stockReviewDetail({id: this.orderId}).then((res) => {
          if (res.code === 0) { // 编辑只有一条商家信息
            this.storeLists.length = 0
            this.storeListSelect[0].items.length = 0
            this.storeLists.push(res.data)
            this.storeLists[0].facadeAddress = res.data.facade_address
            this.storeLists[0].total_val = +res.data.total_val.split(',').join('')
            // 产品信息
            this.storeLists[0].items.map((item) => {
              ...
              this.storeListSelect[0].items.push({productList: [obj]})
            })
            // 凭证信息
            if (this.storeLists[0].fileItems.length > 0) {
              this.storeListSelect[0].defaultList = []
              this.storeLists[0].fileItems.map((item) => {
                item.name = item.title + '.' + item.extension
                this.storeListSelect[0].defaultList.push(item)
              })
            }
            // 商家名称
            this.storeListSelect[0].contractorList.push({
